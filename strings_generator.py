...
IMPORTANT: Archive2.exe references in this module are outdated. As of v2.0,
CC-Packer uses BSArch.exe instead. Archive naming changed from CCMerged to CCPacked.
...
By default, we generate English strings:
  - CCPacked_Main_en.STRINGS
  - CCPacked_Main_en.DLSTRINGS  
  - CCPacked_Main_en.ILSTRINGS

To create translations, simply copy these files and rename with your language code:
  - CCPacked_Main_de.STRINGS (German)
  - CCPacked_Main_fr.STRINGS (French)
  - CCPacked_Main_es.STRINGS (Spanish)
  - CCPacked_Main_it.STRINGS (Italian)
  - CCPacked_Main_ja.STRINGS (Japanese)
  - CCPacked_Main_pl.STRINGS (Polish)
  - CCPacked_Main_pt.STRINGS (Portuguese)
  - CCPacked_Main_ru.STRINGS (Russian)
  - CCPacked_Main_zh.STRINGS (Chinese)
...
from typing import Dict, List, Tuple, BinaryIO, Optional, Callable
import subprocess
import shutil
...
class StringsFile:
    ...
    def read_from_file(cls, filepath: Path) -> 'StringsFile':
        ...
        with open(filepath, 'rb') as f:
            ...
            _data_size = struct.unpack('<I', f.read(4))[0]  # Read but not used - part of file format
    ...
    def extract_strings_from_ba2(self, ba2_path: Path, output_dir: Path) -> Dict[str, Path]:
        ...
        try:
            ...
            _result = subprocess.run(
                ...
                capture_output=True,
                text=True
            )  # Result not used - check=True will raise on failure
...
class CCStringsGenerator:
    ...
    def generate_strings_files(self, esl_names: List[str], progress_callback: Optional[Callable[[str], None]] = None) -> List[Path]:
        """
        Generate STRINGS files for the merged ESL plugins.
        
        Args:
            esl_names: List of ESL filenames (e.g., ["CCPacked_Main.esl"])
            progress_callback: Optional callback for progress updates
    ...
    def generate_for_merged_content(self, esl_names: List[str], cc_ba2_files: List[Path], progress_callback: Optional[Callable[[str], None]] = None) -> List[Path]:
        """
...
def generate_strings_for_merge(fo4_path: str, archive2_path: str, esl_names: List[str], 
                                cc_ba2_files: List[Path], progress_callback: Optional[Callable[[str], None]] = None) -> List[Path]:
    """
    Convenience function to generate STRINGS files for a CC merge operation.
    
    Args:
        fo4_path: Path to Fallout 4 installation
        archive2_path: Path to Archive2.exe (DEPRECATED - v2.0 uses BSArch.exe)
        esl_names: List of ESL filenames (e.g., ["CCPacked_Main.esl"])
        cc_ba2_files: List of original CC BA2 files being merged
        progress_callback: Optional callback for progress updates
